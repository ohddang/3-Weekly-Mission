...
input_emails.input.addEventListener("focusout", () => {
  if (false == sign.isEmailExist(input_emails)) return;
});
...
input_passwords_check.input.addEventListener("focusout", () => {
  if (input_passwords.input.value != input_passwords_check.input.value) return;

  if (false == sign.existTextContent(input_passwords_check)) return;

  if (false == sign.isInvalidPasswordContent(input_passwords_check)) return;
...
input_passwords_check.icon.addEventListener("click", () => {
  sign.showPasswordToggle(
    input_passwords_check.icon,
    input_passwords_check.input
  );
});
